...
if ( isset( $_POST['s2_admin'] ) ) {
	...
	if ( ! empty( $_POST['addresses'] ) ) {
		...
		if ( ! empty( $subscriber ) ) {
			...
			$this->subscribe_registered_users( $export_csv, $s2_request_category );
		}
		...
		if ( ! empty( $subscriber ) ) {
			...
			$this->unsubscribe_registered_users( $export_csv, $s2_request_category );
		}
		...
		if ( ! empty( $subscriber ) ) {
			...
			$this->format_change( $export_csv, $format );
		}
		...
		$sub_category = ! empty( $_POST['sub_category'] ) ? sanitize_text_field( $_POST['sub_category'] ) : '';
		if ( ! empty( $subscriber ) ) {
			$this->digest_change( implode( ",\r\n", $subscriber ), $sub_category );
		} else {
			$this->digest_change( $export_csv, $sub_category );
		}
...
if ( 'registered' === $current_tab ) {
	...
	$all_users  = ! empty( $all_users ) ? $all_users : array();
	$registered = ! empty( $registered ) ? $registered : array();
} else {
	...
	// Safety check for our arrays.
	$confirmed   = ! empty( $confirmed ) ? $confirmed : array();
	$unconfirmed = ! empty( $unconfirmed ) ? $unconfirmed : array();
}
