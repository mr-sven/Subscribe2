=== Subscribe Mini - Form, Email Subscribers & Newsletters ===
Contributors: tareq1988, nizamuddinbabu, wemail, Sven Fabricius
Tags: posts, subscription, email, subscribe, notify, notification, newsletter, post notification, email marketing, optin, form
Requires at least: 4.0
Tested up to: 6.7.1
Stable tag: 1
Requires PHP: 8
License: GPLv3
License URI: https://www.gnu.org/licenses/gpl-3.0.html

Sends a list of subscribers an email notification when you publish new posts.

== Description ==

= Summary =

Subscribe Mini provides a comprehensive subscription management and email notification system for WordPress blogs that sends email notifications to a list of subscribers when you publish new content to your blog.

A full description of features is below.

= Email =

Email notifications can be sent on a per-post basis.

= Subscriptions =

Subscription requests allows users to publicly subscribe (**Public Subscribers**) by submitting their email address in an easy to use form.
Admins are given control over the presentation of the email notifications, can bulk manage subscriptions for users and manually send email notices to subscribers.

The format of the email can also be customised for per-post notifications, Subscribe Mini can generate emails for each of the following formats:

* HTML excerpt.

== Installation ==

= MANUAL INSTALLATION =

1. Copy the entire /subscribe-mini/ directory into your /wp-content/plugins/ directory.
2. Activate the plugin.
3. Visit the "Subscribe Mini -> Settings" menu.
4. Configure the options to taste, including the email template and any categories which should be excluded from notification
5. Visit the "Subscribe Mini-> Subscribers" menu.
6. Manually subscribe people as you see fit.
7. Create a [WordPress Page](http://codex.wordpress.org/Pages) to display the subscription form. Insert the Subscribe Mini shortcode or token: [subscribe-page]. ***Ensure the token is on a line by itself and that it has a blank line above and below.***
This token will automatically be replaced by dynamic subscription information and will display all forms and messages as necessary.
8. In the WordPress "Settings" area for Subscribe Mini select the page name in the "Options" section that of the WordPress page created in step 7.
