...
/**
 * Subscribe2 upgrade handler class.
 */
class S2_Upgrade {
	...
	/**
	 * Subscribe core upgrade function.
	 *
	 * @return void
	 */
	private function upgrade_core() {
		...
		if ( version_compare( $mysubscribe2->wp_release, '3.5', '<' ) ) {
			...
			$users = $wpdb->get_col(
				$wpdb->prepare(
					"SELECT ID from $wpdb->users WHERE ID NOT IN (SELECT user_id FROM $wpdb->usermeta WHERE meta_key=%s)",
					$mysubscribe2->get_usermeta_keyname( 's2_format' )
				)
			);
	...
	/**
	 * Subscribe core upgrader function.
	 *
	 * @return void
	 */
	...
	private function upgrade7_0() {
		...
		if ( version_compare( $mysubscribe2->wp_release, '3.5', '<' ) ) {
			$users = $wpdb->get_col(
				$wpdb->prepare(
					"SELECT ID from $wpdb->users WHERE ID NOT IN (SELECT user_id from $wpdb->usermeta WHERE meta_key=%s",
					$mysubscribe2->get_usermeta_keyname( 's2_authors' )
				)
			);
